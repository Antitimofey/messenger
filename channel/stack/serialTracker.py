...
class SerialTracker:
    def __init__(self, rx: COMPort):
        self.rx = rx
        self.listening_tread = threading.Thread(target=self.receive_message, daemon=True)
        self.queue = Queue()

    def receive_message(self):
        while True:

            # сдесь надо положить сообщение в self.queue

            bytes_frame = rx.receive_bytes()
            message = EncryptedFrame(raw_bytes=bytes_frame)
    ...
    def start_listening(self):
        assert self.rx.open_port(), "необходимо сначала открыть порт!"
        self.listening_tread.start()
    
    def stop_listening(self):
        self.listening_tread.stop()
